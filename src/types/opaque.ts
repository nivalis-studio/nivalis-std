...
A type returned by `Opaque` can be passed to `Opaque` again, to create a type with multiple tags.
...
A type `A` returned by `Opaque` is assignable to another type `B` returned by `Opaque` if and only if:
...
@example
```
// You can apply multiple tags to a type by using `Opaque` repeatedly.
type Url = Opaque<string, 'URL'>;
type SpecialCacheKey = Opaque<Url, 'SpecialCacheKey'>;

// You can also pass a union of tag names, so this is equivalent to the above, although it doesn't give you the ability to assign distinct metadata to each tag.
type SpecialCacheKey2 = Opaque<string, 'URL' | 'SpecialCacheKey'>;
```
*/
export type Opaque<
  Type,
