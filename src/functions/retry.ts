 ...
 * @param {number | ((attempts: number) => number)} [options.delay] - Delay(milliseconds) between retries.
 * @param {number} [options.retries] - The number of retries to attempt.
...
export async function retry<T>(
  ...
  for (let attempts = 0; attempts < retries; attempts++) {
    ...
    try {
      ...
      const currentDelay =
        typeof delay === 'function' ? delay(attempts) : delay;
