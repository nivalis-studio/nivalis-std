...
export function overEvery<T>(
  ...predicates: Array<
    ((...args: T[]) => boolean) | ReadonlyArray<(...args: T[]) => boolean>
  >
): (...args: T[]) => boolean;
...
export function overEvery<T>(
  ...predicates: Array<
    ((...values: T[]) => boolean) | ReadonlyArray<(...values: T[]) => boolean>
  >
): (...values: T[]) => boolean {
  return function (this: any, ...values: T[]) {
    for (const predicate of predicates) {
      ...
      for (const element of predicate) {
        if (!createIteratee(element).apply(this, values)) {
          return false;
        }
      }
    }
