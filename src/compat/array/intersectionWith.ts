...
export function intersectionWith<T>(
  firstArr: ArrayLike<T> | null | undefined,
  ...otherArrs: Array<
    ArrayLike<T> | null | undefined | ((x: T, y: T) => boolean)
  >
): T[];
...
export function intersectionWith<T>(
  ...
  let result = uniq([...firstArr]);

  for (const otherArr_ of otherArrs) {
    const otherArr = otherArr_ as ArrayLike<T>;

    if (otherArr == null) {
      return [];
    }

    result = intersectionWithToolkit(result, [...otherArr], comparator);
  }
