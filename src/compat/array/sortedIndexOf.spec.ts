...
describe('sortedIndexOf', () => {
  ...
  it(`should return \`-1\` for an unmatched value`, () => {
    ...
    expect(sortedIndexOf(empty)).toBe(-1);
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-expect-error
  ...
  it(`should not match values on empty arrays`, () => {
    ...
    expect(sortedIndexOf(array)).toBe(-1);
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-expect-error
  ...
  it(`should match \`NaN\``, () => {
    const sortedArray = [1, 2, Number.NaN, Number.NaN];

    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-expect-error
    expect(sortedIndexOf(sortedArray, Number.NaN, true)).toBe(2);
  });
  ...
  it('should not handle array with NaN Searching Number', () => {
    const array = [Number.NaN, 2, Number.NaN, 3, 4];
  ...
  it('should handle undefined values', () => {
    ...
    expect(sortedIndexOf(array)).toBe(0);
    expect(sortedIndexOf(array, 1)).toBe(-1);
  });

  it('should handle NAN values', () => {
    const array = [0, 1, Number.NaN, Number.NaN, Number.NaN, 3];

    expect(sortedIndexOf(array, Number.NaN)).toBe(2);
  ...
  it('should handle mixed types with undefined correctly', () => {
    ...
    expect(sortedIndexOf(arr)).toBe(-1);
    expect(sortedIndexOf(arr, Symbol('test'))).toBe(-1);
  });

  // Mixed types ver2
  it('should handle mixed types with midVal is NaN correctly', () => {
    const arr = [undefined, null, Number.NaN, Symbol('test'), 1];

    expect(sortedIndexOf(arr, null)).toBe(-1);
    expect(sortedIndexOf(arr)).toBe(-1);
    expect(sortedIndexOf(arr, Number.NaN)).toBe(2);
  });
});
