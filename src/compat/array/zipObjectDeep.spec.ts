...
describe('zipObjectDeep', () => {
  it('creates an object from two arrays of keys and values', () => {
    ...
    expect(zipObjectDeep(symbols, [1, 2])).toEqual({
      [symbols[0]]: 1,
      [symbols[1]]: 2,
    });
    ...
    expect(zipObjectDeep(['a', 'b', 'c'], [1, 2])).toEqual({
      a: 1,
      b: 2,
      c: undefined,
    });
    expect(zipObjectDeep(['a.b.c', 'd.e.f'], [1, 2])).toEqual({
      a: { b: { c: 1 } },
      d: { e: { f: 2 } },
    });
    expect(zipObjectDeep(['a.b.c', 'a.b.d', 'a.b.e.f'], [1, 2, 3])).toEqual({
      a: { b: { c: 1, d: 2, e: { f: 3 } } },
    });
