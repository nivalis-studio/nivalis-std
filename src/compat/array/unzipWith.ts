...
export function unzipWith<T>(
  ...
  const unziped = isArray(array)
    ? unzipToolkit(array)
    : unzipToolkit(Array.from(array, value => [...value]));

  if (!iteratee) {
    return unziped;
  }

  const result: any[] = Array.from({ length: unziped.length });

  for (const [i, value] of unziped.entries()) {
