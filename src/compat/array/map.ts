 ...
 * map(numbers); // => [1, 2, 3]
 */
export function map<T>(collection: readonly T[], iteratee?: null): T[];
 ...
 */
export function map<T>(collection: ArrayLike<T>, iteratee?: null): ArrayLike<T>;
...
export function map<T>(
  object: T,
  iteratee: [keyof T[keyof T], unknown],
): boolean[];
 ...
 * map(obj, 'x'); // => [1, 2, 3]
 */
export function map<T, K extends keyof T[keyof T]>(
  object: T,
  iteratee: K,
): Array<T[keyof T][K]>;
 ...
 * map(obj); // => [1, 2, 3]
 */
export function map<T extends object, U>(object: T, iteratee?: null): U[];
...
export function map(
  ...
  _iteratee?:
    | ((value: any, index: PropertyKey, collection: any) => any)
    | PropertyKey
    | object
    | null,
): any[] {
