...
export function transform<T, U>(
  object: { [key: string]: T },
  iteratee?: (acc: U, curr: T, key: string, dict: { [key: string]: T }) => void,
  accumulator?: U,
): U;
...
export function transform<T, U>(
  ...
  accumulator?: U,
): U | any[] | { [key: string]: any } {
  const isArrayOrBufferOrTypedArray =
    Array.isArray(object) || isBuffer(object) || isTypedArray(object);
  ...
  forEach(object, (value, key, object) =>
    iteratee(accumulator as U, value as T, key, object),
  );
